(async()=>{
  ...
  const finalizer='<scr'+'ipt>(function(){const started=Date.now();let loginSince=0,joinSince=0,revealed=false;function visible(e){return !!e&&!e.classList.contains("hide")&&getComputedStyle(e).display!=="none"&&getComputedStyle(e).visibility!=="hidden"}function reveal(){if(revealed)return;revealed=true;const s=document.getElementById("nc-startup-shield");if(!s)return;s.classList.add("nc-out");setTimeout(()=>{s.remove();document.getElementById("nc-startup-style")?.remove()},240)}setTimeout(reveal,4200);async function tick(){if(revealed)return;const s=document.getElementById("nc-startup-shield");if(!s)return;const u=new URL(location.href),wantsRoom=!!u.searchParams.get("room");const app=document.getElementById("app"),home=document.getElementById("nc20home"),login=document.querySelector(".n19a"),join=document.getElementById("join");if(wantsRoom&&visible(app)){reveal();return}if(!wantsRoom&&home){reveal();return}let hasSession=null;try{if(window.NovaCallAccountClient){const r=await window.NovaCallAccountClient.auth.getSession();hasSession=!!(r?.data?.session||r?.data?.user)}}catch{}if(login&&visible(login)){if(hasSession===false){if(!loginSince)loginSince=Date.now();if(Date.now()-loginSince>350){reveal();return}}else loginSince=0}else loginSince=0;if(!wantsRoom&&visible(join)&&!login&&!home){if(hasSession===false||Date.now()-started>2500){if(!joinSince)joinSince=Date.now();if(Date.now()-joinSince>250){reveal();return}}else joinSince=0}else joinSince=0;setTimeout(tick,100)}setTimeout(tick,80)})();</scr'+'ipt>';
